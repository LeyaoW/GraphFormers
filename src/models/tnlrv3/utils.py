...
class Seq2seqDatasetForTuringNLRv3(torch.utils.data.Dataset):
    ...
    def __getitem__(self, _idx):
        ...
        source_ids = self.__trunk([self.cls_id] + feature.source_ids, self.max_source_len, append_sep=self.finetuning_method != 'v0')
        target_ids = feature.target_ids
        if self.finetuning_method == 'v0':
            target_ids = [self.sep_id] + target_ids
        target_ids = self.__trunk(target_ids, self.max_target_len, append_sep=self.finetuning_method != 'v0')
